...
const OwnerDashboard = () => {
  ...
  const handleToggleOpen = async () => {
    if (!shop?._id) return alert("Shop data not ready");
    try {
      const res = await axios.patch(
        `${serverURL}/api/shop/${shop._id}/open`,
        {},
        { withCredentials: true }
      );
      if (res.data.success) {
        const updatedShop = { ...shop, isOpen: res.data.isOpen };
  ...
  const handleToggleDelivery = async () => {
    ...
    try {
      const res = await axios.patch(
        `${serverURL}/api/shop/${shop._id}/delivery`,
        {},
        { withCredentials: true }
      );
      if (res.data.success) {
      ...
      console.error("Toggle delivery failed", err);
      alert("Failed to toggle delivery status");
    }
  };

  const totalRevenue =
    (Array.isArray(myShopOrders)
      ? myShopOrders.reduce((acc, order) => {
          const shopOrder = (order.shopOrders || []).find((so) => {
            const ownerId = so?.owner?._id ?? so?.owner;
            const myId = userData?._id ?? userData?.id;
            return ownerId === myId;
          });
          return acc + Number(shopOrder?.subtotal || 0);
        }, 0)
      : 0) || 0;

  const pendingOrders =
    (Array.isArray(myShopOrders)
      ? myShopOrders.filter((order) => {
          const shopOrder = (order.shopOrders || []).find((so) => {
            const ownerId = so?.owner?._id ?? so?.owner;
            const myId = userData?._id ?? userData?.id;
            return ownerId === myId;
          });
          return shopOrder?.status === "Pending";
        }).length
      : 0) || 0;
