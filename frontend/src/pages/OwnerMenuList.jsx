...
import { Edit2, Trash2, Eye, Lock, Clock, ArrowLeft } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex justify-between items-start">
            <div>
              <button
                onClick={() => navigate("/")}
                className="mb-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold transition-colors"
              >
                <ArrowLeft size={16} />
                Back
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.map((item) => (
              <OwnerItemCard
                ...
                onViewMenu={() => navigate(`/owner/view-item/${item._id}`)}
              />
            ))}
          </div>
        )}
...
const OwnerItemCard = ({ item, colors, onEdit, onDelete, onToggleAvailability, onViewMenu }) => {
  ...
  return (
    ...
    >
      {/* Image + status badge: badge must live inside this relative box so position:absolute is anchored correctly */}
