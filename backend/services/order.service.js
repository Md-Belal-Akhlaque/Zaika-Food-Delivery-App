...
const ONLINE_PAYMENT_STOCK_HOLD_MS =
  Number(process.env.ORDER_STOCK_HOLD_MINUTES || 15) * 60 * 1000;
...
const releaseStockForShopOrder = async (shopOrder, session = null) => {
...
export const releaseReservedStockForOrder = async ({
  ...
  if (!order?._id) {
    return { released: false, reason: "order_missing" };
  }

  let targetOrder = order;

  const hasPopulatedShopOrders =
    Array.isArray(targetOrder.shopOrders) &&
    targetOrder.shopOrders.length > 0 &&
    typeof targetOrder.shopOrders[0] === "object" &&
    Array.isArray(targetOrder.shopOrders[0]?.items);

  if (!hasPopulatedShopOrders) {
    let query = Order.findById(targetOrder._id).populate("shopOrders");
    if (session) {
      query = query.session(session);
    }
    targetOrder = await query;
  }
  ...
  if (targetOrder.paymentMethod !== "online") {
    return { released: false, reason: "not_online_order" };
  }

  if (targetOrder.paymentStatus === "Paid") {
    return { released: false, reason: "already_paid" };
  }

  if (targetOrder.isStockReleased) {
    return { released: false, reason: "already_released" };
  }

  for (const shopOrder of targetOrder.shopOrders || []) {
    await releaseStockForShopOrder(shopOrder, session);
  }

  targetOrder.isStockReleased = true;
  targetOrder.stockReleasedAt = new Date();
  targetOrder.stockReleaseReason = reason || "payment_failed";
  targetOrder.stockReleaseAt = null;

  if (targetOrder.paymentStatus === "Pending") {
    targetOrder.paymentStatus = "Failed";
    targetOrder.paymentFailedAt = targetOrder.paymentFailedAt || new Date();
    targetOrder.paymentFailureReason =
      targetOrder.paymentFailureReason ||
      (reason === "stock_hold_ttl_expired"
        ? "Payment timeout - stock released"
        : "Payment failed - stock released");
  }

  await targetOrder.save({ session });
  return { released: true, reason: targetOrder.stockReleaseReason };
};
...
export const createOrderService = async ({ userId, cartItems, deliveryAddress, paymentMethod, idempotencyKey }) => {
  ...
  try {
    ...
    const order = new Order({
      ...
      stockReservedAt: paymentMethod === "online" ? new Date() : null,
      stockReleaseAt:
        paymentMethod === "online"
          ? new Date(Date.now() + ONLINE_PAYMENT_STOCK_HOLD_MS)
          : null,
